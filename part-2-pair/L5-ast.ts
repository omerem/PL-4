...
export const isCompoundExp = (x: any): x is CompoundExp =>
    ...
    isLetExp(x) || isLetrecExp(x) || isSetExp(x) || isPairExp(x);
...
export interface PairExp {tag: "PairExp"; comp1: CExp; comp2: CExp}
export const makePairExp = (comp1: CExp, comp2: CExp) : PairExp =>
    ({tag: "PairExp", comp1: comp1, comp2: comp2});
export const safeMakePairExp = (comp1: CExp | Error, comp2: CExp | Error) : PairExp | Error =>
    isError(comp1) ? comp1 :
    isError(comp2) ? comp2 :
    makePairExp(comp1, comp2);
...
export const makeLitExp
 = (val: SExp): LitExp => ({tag: "LitExp", val: val});
...
export const parse = (x: string): Parsed | Error =>
{
    ...
    return parseSexp(p(x));
    
    // const y = parseSexp(p(x));
    // if(isCompoundSExp(y))
    // if(y.val)
    // if(isArray(y.val))
    // if(y.val.length === 3)
    // if(deepEqual(y.val, {tag: 'SymbolSExp', val: '.'}))
    // makePairExp(parseCExp(parseSExp(y.val[0])), y.val[2]);
...
const parseCompoundCExp = (sexps: any[]): CExp | Error =>
    ...
    first(sexps) === "quote" ? 
                        (( second(second(sexps)) === "." && second(sexps).length === 3 ) ? 
                            parsePairExp(["Pair",second(sexps)[0],second(sexps)[2]]) : 
                            parseLitExp(sexps)) :
    first(sexps) === "Pair" ? parsePairExp(sexps) :
    parseAppExp(sexps)
...
const parsePairExp = (sexps: any[]): PairExp | Error => {
    if (sexps.length !== 3)
    return Error(`Too many or too few expressions in - ${sexps}`);
    if (sexps[0] === "Pair")
    {
        const comp1: CExp | Error = parseCExp(sexps[1]);
        const comp2: CExp | Error = parseCExp(sexps[2]);
        return safeMakePairExp(comp1, comp2);
    }
    else if(sexps[1] === "."){
        const comp1: CExp | Error = parseCExp(sexps[0]);
        const comp2: CExp | Error = parseCExp(sexps[2]);
        return safeMakePairExp(comp1, comp2);
    } else
    return Error(`Not a valid pairExp: ${sexps}`);
}
...
export const parseSExp = (x: any): SExp | Error =>
    ...
    //todo
    // (isArray(x) && (x.length === 3) && (x[1] === ".")) ? makePairExp(parse(x[0]), parseSExp(x[2])) :
...
export const unparse = (e: Parsed | Error): string | Error =>
    ...
    isPairExp(e) ? `(Pair ${unparse(e.comp1)} ${unparse(e.comp2)})` :
