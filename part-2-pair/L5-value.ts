...
import { isPrimOp, CExp, PrimOp, VarDecl,isArray, PairExp, isPairExp, makePairExp, parseCExp } from './L5-ast';
...
export type SExp = number | boolean | string | PrimOp | Closure | SymbolSExp | EmptySExp | CompoundSExp | PairExp;
export const isSExp = (x: any): x is SExp =>
    typeof(x) === 'string' || typeof(x) === 'boolean' || typeof(x) === 'number' ||
    isSymbolSExp(x) || isCompoundSExp(x) || isEmptySExp(x) || isPrimOp(x) || isClosure(x) || isPairExp(x);
