...
import { isAppExp, isBoolExp, isDefineExp, isEmpty, isIfExp, isLetrecExp, isLetExp, isNumExp,
         isPrimOp, isProcExp, isPairExp, makeLitExp,parseLitExp, isProgram, isStrExp, isVarRef, parse, unparse,
         AppExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, LitExp, NumExp, isLitExp,
         Parsed, PrimOp, ProcExp, PairExp, Program, SetExp, StrExp, parseCExp } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp, makePairTExp, isPairTExp,
         BoolTExp, NumTExp, ProcTExp, StrTExp, TExp, PairTExp, makeLitTExp, parseTExp } from "./TExp";
import { getErrorMessages, hasNoError, isError } from './error';
import { allT, first, rest, second } from './list';
import { SExp } from "./L5-value";
import { isSymbolSExp, isEmptySExp, isClosure, isSExp , isCompoundSExp} from "./L5-value";
import { isCompoundExp } from "./L5-ast";
import { parseSExp } from "./L5-ast";
...
export const typeofExp = (exp: Parsed | Error, tenv: TEnv): TExp | Error =>
    ...
    isLitExp(exp) ? typeofLit_PairTExp(exp, tenv) :
...
export const typeofPair = (pair: PairExp, tenv: TEnv): TExp | Error => {
    const comp1T = typeofExp(pair.comp1, tenv);
    const comp2T = typeofExp(pair.comp2, tenv);
    if (isError(comp1T)) return comp1T;
    if (isError(comp2T)) return comp2T;
    return makePairTExp(comp1T, comp2T);
}

export const typeofLit_PairTExp = (lit: LitExp, tenv: TEnv) : TExp | Error =>{
    const litVal = lit.val;
    
    if(isError(litVal)) return litVal;
    if(isPairExp(litVal)) {
        const valT = typeofExp(litVal, tenv);
        if(isPairTExp(valT)) return makeLitTExp(valT);
        else return Error(`Expected: pairExp. Got: ${lit.val}`);
    }
    else return Error(`Expected: pairExp. Got: ${lit.val}`);
}
