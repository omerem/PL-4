...
import { L5typeof } from "../part-2-define-program/L5-typecheck";
...
export const inferType = (exp: A.Exp): T.TExp => {
    ...
    if (T.isTVar(texp) && ! isError(sub)){
        const texpSubGet = S.subGet(sub, texp);
...
const solve = (equations: Equation[], sub: S.Sub): S.Sub | Error => {
    ...
    if(T.isTVar(eq.left)){
        const solvedVarEq = solveVarEq(eq.left, eq.right);
        return solvedVarEq;
    }
    else if(T.isTVar(eq.right)){
        return solveVarEq(eq.right, eq.left);
    }
    else if(bothSidesAtomic(eq)){
        return handleBothSidesAtomic(eq);
    }
    else if(T.isCompoundTExp(eq.left) && T.isCompoundTExp(eq.right) && canUnify(eq)){
        return solve(R.concat(rest(equations), splitEquation(eq)), sub);
    }
    else return  Error(`Equation contains incompatible types ${eq}`);
...
const splitEquation = (eq: Equation): Equation[] =>
{
    ...
    else if (T.isPairTExp(eq.left) && T.isPairTExp(eq.right))
    {
        ...
        return R.zipWith(makeEquation,
            [eq.left.comp1T, eq.left.comp2T],
            [eq.right.comp1T, eq.right.comp2T]);
        }
    else
    return [];
