...
import { isArray, isBoolean, isEmpty, isString, DefineExp } from './L5-ast';
...
export type CompoundTExp = ProcTExp | TupleTExp | DefineTExp;
export const isCompoundTExp = (x: any): x is CompoundTExp => isProcTExp(x) || isTupleTExp(x) || isDefineTExp(x);

export type NonTupleTExp = AtomicTExp | ProcTExp | TVar | DefineTExp;
export const isNonTupleTExp = (x: any): x is NonTupleTExp =>
    isAtomicTExp(x) || isProcTExp(x) || isTVar(x) || isDefineTExp(x);
...
export type DefineTExp = {tag: "DefineTExp"; varT: TExp;}
export const makeDefineTExp = (varT: TExp): DefineTExp =>
({tag: "DefineTExp", varT: varT});
export const isDefineTExp = (x: any): x is DefineTExp => x.tag === "DefineTExp";
