...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, ProcTExp, StrTExp, TExp, isDefineTExp } from "./TExp";
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): TExp | Error => {
    ...
    let varT = exp.var.texp;
    let valT = typeofExp(exp.val,
        makeExtendTEnv([exp.var.var], [varT], tenv));
    // console.log(`varT: ${varT}\nvalT: ${valT}`)
    const constraint1 = checkEqualType(varT, valT, exp);
    const constraint2 = !isDefineTExp(varT);

    if (isError(constraint1)) return constraint1;
    if (!constraint2) 
    return   Error(`Can't define inside defExp`);
...
export const typeofProgram = (exp: Program, tenv: TEnv): TExp | Error => {
    ...
    return Error(`Program: ${unparse(exp)} is an empty program.`);
    let firstExp = exp.exps[0];
    if(exp.exps.length === 1) return typeofExp(firstExp, tenv);
    
    if(isDefineExp(firstExp)){
        
        return typeofProgram(makeProgram(exp.exps.slice(1)),
        makeExtendTEnv([firstExp.var.var],[firstExp.var.texp],tenv));
    }
    return typeofProgram(makeProgram(exp.exps.slice(1)),tenv);
